...
def create_plot(regular, super_fuel, diesel):
    ...
    colors = {'Regular': '#abdfff', 'Super': '#0091e4', 'Diesel': '#00224f'}  # Blue tones
    bar_plot = filtered_df.hvplot.bar(x='Tipo de gasolina', y='Amount', color=filtered_df['Tipo de gasolina'].map(colors), alpha=0.6, label='Promedio de gasolina por tipo', width=700)
...
def create_plot2(regular, super_fuel, diesel):
    ...
    if regular:
        ...
        plots.append(hv.Curve(monthly_averages, label='Regular').opts(color='#abdfff'))  # Dark blue
        
    if super_fuel:
        df = read_csv_data('data/gassuperpormes.csv')
        monthly_averages = df.mean(axis=1, skipna=True)
        plots.append(hv.Curve(monthly_averages, label='Super').opts(color='#0091e4'))  # Light blue
        
    if diesel:
        df = read_csv_data('data/dieselpormes.csv')
        monthly_averages = df.mean(axis=1, skipna=True)
        plots.append(hv.Curve(monthly_averages, label='Diesel').opts(color='#00224f'))  # Lighter blue
        
    if not plots:
        return hv.Curve([])  # Empty plot if none are selected
    
    overlay = hv.Overlay(plots).opts(width=1200, xlabel='Mes', ylabel='Precio', title='Promedios mensuales')
    return overlay

@pn.depends(regular_checkbox.param.value, super_checkbox.param.value, diesel_checkbox.param.value)
def create_plot3(regular, super_fuel, diesel):
    plots = []
    
    if regular:
        df = read_csv_data('data/gasregularpormes.csv')
        monthly_averages = df.mean(axis=1, skipna=True)
        plots.append(hv.Bars(monthly_averages, label='Regular').opts(color='#abdfff'))
        
    if super_fuel:
        df = read_csv_data('data/gassuperpormes.csv')
        monthly_averages = df.mean(axis=1, skipna=True)
        plots.append(hv.Bars(monthly_averages, label='Super').opts(color='#0091e4'))
        
    if diesel:
        df = read_csv_data('data/dieselpormes.csv')
        monthly_averages = df.mean(axis=1, skipna=True)
        plots.append(hv.Bars(monthly_averages, label='Diesel').opts(color='#00224f'))
        
    if not plots:
        return hv.Bars([])  # Empty plot if none are selected
    
    overlay = hv.Overlay(plots).opts(width=1200, xlabel='Month', ylabel='Average Value', title='Monthly Averages')
    return overlay
...
bound_plot3 = pn.bind(create_plot3, regular=regular_checkbox, super_fuel=super_checkbox, diesel=diesel_checkbox)
...
dynamic_plot3 = hv.DynamicMap(bound_plot3)

# Combine the two dynamic plots in a column layout
combined_plot = pn.Column(dynamic_plot1, dynamic_plot2, dynamic_plot3)
